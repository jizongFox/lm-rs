...
class SimpleSSIMFunction(Function):
    ...
    def forward(ctx, img1, img2, window, window_size, channel, GPeak):
        ...
        ctx.save_for_backward(
            img1, img2, mu1, mu2, mu1_sq, mu2_sq, sigma1_sq, sigma2_sq
        )
        ctx.window = window
    ...
    def backward(ctx, grad_output):
        ...
        delLuminance_delPixel = (
            2 * GPeak * ((mu2 - mu1 * luminance) / (mu1_sq + mu2_sq + C1))
        )

        d_contrast_structure = (img2 - mu2) - contrast_structure * (img1 - mu1)
        delContrastStructure_delPixel = (2 * GPeak * d_contrast_structure) / (
...
def simple_ssim_loss(rendered, gt, lambda_ssim=1.0, channel=3, window_size=11):
    ...
    ssim_map = SimpleSSIMFunction.apply(
        rendered, gt, window, window_size, channel, GPeak
    )
    return ssim_map
