...
from arguments import ModelParams, PipelineParams, OptimizationParams, GaussNewtonOptimizationParams, SchedulerParameters
import numpy as np
...
def densify_prune_adam(opt, gaussians:GaussianModel, scene, iteration, visibility_filter, radii, viewspace_point_tensor, optimization_method, 
    ...
    if  iteration < opt.densify_until_iter:
        ...
        if iteration > opt.densify_from_iter and iteration  % opt.densification_interval == 0:
            ...
            gaussians.densify_and_prune(opt.densify_grad_threshold, 0.005, scene.cameras_extent, size_threshold, optimization_method)
            print(f"Iter {iteration} Number of Gaussians: ", gaussians._xyz.shape[0])
            print("Memory: ", torch.cuda.max_memory_allocated() / 1e9, " GB")
        
        if iteration % (opt.opacity_reset_interval) == 0: # or (dataset.white_background and iteration == opt.densify_from_iter) I dont use white
            gaussians.reset_opacity(optimization_method)

def save_tensors(gaussians:GaussianModel, params:GaussNewtonOptimizationParams, optimizerHandler, path:str):
...
def training(dataset, opt, pipe, log_dict, saving_iterations, checkpoint_iterations, checkpoint, debug_from, optimization_method:str, gn_opt_params:GaussNewtonOptimizationParams,
            enable_densify:bool, save:bool, sched_params=None):
    ...
    optimization_method = "first_order" if optimization_method in ["adam", "rmsprop", "sgd"] else optimization_method
    optimizerHandler = OptimizerHandler(gaussians, optimization_method, gn_opt_params, opt, scene.model_path, scene.cameras_extent).getOptimizer()
    # optimizerHandler_adam = OptimizerHandler(gaussians, "adam", gn_opt_params, opt, scene.model_path).getOptimizer()

    camera_sampler_dict = {"batch_size": gn_opt_params.batch_size, "path": scene.model_path}
    camera_sampler = CameraSampler.samplers[gn_opt_params.camera_sampler](scene.getTrainCameras(), camera_sampler_dict)
    ...
    with torch.no_grad():
        training_report(tb_writer, 1, 0, log_dict, 
                                scene, render, (pipe, background), optimization_method, gaussians, dataset.model_path)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if optimization_method == "first_order":
            ...
            loss, elapsed, viewspace_point_tensor, visibility_filter, radii  = adam_step(camera_sampler, iteration, debug_from, opt, scene, viewpoint_stack, gaussians, pipe, background, optimizerHandler, batch_size)

        elif optimization_method == "cg-gpu":
            if (gaussians.cgState.kernel == -1):
                save_jacob(gn_opt_params.batch_size, scene, viewpoint_stack, gaussians, pipe, background, camera_sampler, dataset.model_path)
            else:
                if not gn_opt_params.disable_scheds:
                    gaussians.scheduler_step(iteration, camera_sampler, optimizerHandler)
                batch_size = gaussians.cgState.batch_size
                loss, elapsed  = gauss_newton_step(pixel_sampler, camera_sampler, iteration, debug_from, opt, gaussians, pipe, background, optimizerHandler, gn_opt_params)
        ...
        with torch.no_grad():
            ...
            training_report(tb_writer, iteration, total_elapsed, log_dict, 
                            scene, render, (pipe, background), optimization_method, gaussians, dataset.model_path)
            ...
            if (iteration in checkpoint_iterations):
                torch.save((gaussians.capture(optimization_method), iteration), scene.model_path + "/chkpnt" + str(iteration) + ".pth")
...
def training_report(tb_writer, iteration, elapsed, log_dict, scene : Scene, 
            renderFunc, renderArgs, optimization_method, gaussians, path):
    ...
    if (iteration in testing_iterations):
        ...
        selected_cameras = [train_cameras[i * step] for i in range(num_selected_cameras)]
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                print("\n[ITER {}] Evaluating {}: L2 {} SSIM {} PSNR {} LPIPS {}".format(iteration, config['name'], l2_test, ssim_test, psnr_test, lpips_test))
                print(f"Number of Gaussians is : {gaussians._xyz.shape[0]}")
                print("Forward + Backward Time ", elapsed, flush=True)
                if tb_writer:                    
                    tb_writer.add_scalar(config['name'] + '/optim_steps - ssim', ssim_test, iteration, walltime=elapsed)
                    tb_writer.add_scalar(config['name'] + '/optim_steps - l2', l2_test, iteration, walltime=elapsed)
                    tb_writer.add_scalar(config['name'] + '/optim_steps - psnr', psnr_test, iteration, walltime=elapsed)
                    tb_writer.add_scalar(config['name'] + '/optim_steps - lpips', lpips_test, iteration, walltime=elapsed)
                ...
                np.save( os.path.join(metric_path, f"{config['name']}_ssim.npy"), np.asarray(ssim_list[config['name']], dtype=np.float32) )
                np.save( os.path.join(metric_path, f"{config['name']}_time.npy"), np.asarray(time_list[config['name']], dtype=np.float32) )
                np.save( os.path.join(metric_path, f"{config['name']}_mse.npy"), np.asarray(l2_list[config['name']], dtype=np.float32) )
                np.save( os.path.join(metric_path, f"{config['name']}_psnr.npy"), np.asarray(psnr_list[config['name']], dtype=np.float32) )
                np.save( os.path.join(metric_path, f"{config['name']}_lpips.npy"), np.asarray(lpips_list[config['name']], dtype=np.float32) )
                np.save( os.path.join(metric_path, f"{config['name']}_iter.npy"), np.asarray(step_list[config['name']], dtype=np.int32) )
        ...
        if tb_writer:
            ...
            if log_images:
                ...
                tb_writer.add_histogram("scene/color_histogram", scene.gaussians._features_dc.flatten(), iteration)
                tb_writer.add_histogram("scene/xyz_histogram", scene.gaussians._xyz.flatten(), iteration)
                tb_writer.add_histogram("scene/scale_histogram", scene.gaussians._scaling.flatten(), iteration)
...
if __name__ == "__main__":
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), log_dict, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from,
                    args.optimization_method, gn_op.extract(args), args.enable_densify, args.save, sched_opts)
