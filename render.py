...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, video:bool):
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree, optimization_method="cg-gpu")
        ...
        if not skip_test:
             render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, video)
