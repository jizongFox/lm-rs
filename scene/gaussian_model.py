...
class BatchState:
    ...
    def update(self, key, new_value):
        if key == "batchsize_sched" and new_value != self.batch_size:
            ...
            self.radii_batch = torch.zeros(
                (self.batch_size, self.n_gaussians), device="cuda"
            )
...
class GaussianModel:
    ...
    def create_from_pcd(
        self, pcd: BasicPointCloud, spatial_lr_scale: float, scale_cutoff=10000
    ):
        ...
        self.batch_visibility_filter = torch.zeros(
            (self.get_xyz.shape[0]), device="cuda", dtype=torch.bool
        )
    ...
    def training_setup(self, training_args, gn_opt_params):
        ...
        if self.optimization_method == "cg-gpu":
            ...
            self.batchState = BatchState(
                self.cgState.batch_size, self.get_gaussian_count()
            )
    ...
    def cat_tensors_direct(self, d):
        ...
        self._features_dc = torch.cat(
            (self._features_dc, d["f_dc"]), dim=0
        ).requires_grad_(True)
        self._features_rest = torch.cat(
            (self._features_rest, d["f_rest"]), dim=0
        ).requires_grad_(True)
        self._opacity = torch.cat((self._opacity, d["opacity"]), dim=0).requires_grad_(
            True
        )
        self._scaling = torch.cat((self._scaling, d["scaling"]), dim=0).requires_grad_(
            True
        )
        self._rotation = torch.cat(
            (self._rotation, d["rotation"]), dim=0
        ).requires_grad_(True)
    ...
    def densify_and_split(
        self, grads, grad_threshold, scene_extent, optimization_method, N=2
    ):
    ...
    def split_based_on_scale(self, N=2):
        ...
        selected_pts_mask = torch.where(
            self.get_scaling >= scale_threshold, True, False
        )
    ...
    def densify_and_clone(
        self, grads, grad_threshold, scene_extent, optimization_method="adam"
    ):
        # Extract points that satisfy the gradient condition
        if optimization_method == "adam":
            selected_pts_mask = torch.where(
    ...
    def densify_and_prune(
        self, max_grad, min_opacity, extent, max_screen_size, optimization_method="adam"
    ):
    ...
    def add_densification_stats_v2(self, mean2Dgrad, update_filter):
        ...
        self.xyz_gradient_accum[update_filter] += torch.norm(
            mean2Dgrad[update_filter], dim=-1, keepdim=True
        )
        self.denom[update_filter] += 1
