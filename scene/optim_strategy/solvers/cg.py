...
class CGSolver(BaseSolver):
    def __init__(self, size, solver="CG", linear_iter=1, lambda_reg=1e-2, levenberg_type="identity"):
        super().__init__(solver, linear_iter, lambda_reg)
        self.init_solver(size)
        self.size = size // 14 # There are 14 params, 3 location, 3 scale, 4 rot, 3 color, 1 opac
        assert levenberg_type == "identity" or levenberg_type == "diagonal", "Levenberg Regularizer is not supported" 
