...
class CGOptimizer():
    ...
    def __init__(self, gaussians, gn_opt_params :GaussNewtonOptimizationParams, path, cameras_extend) -> None:
        ...
        delSSIM_delPixel = torch.zeros((self.batch_size, W*H*CHANNELS), device="cuda")
        self.residual_state = ResidualState(deltaY_batch, ssim_residuals, delSSIM_delPixel)
        ...
        self.solver = solver_class(gaussians._xyz.shape[0] * GAUSSIAN_ATTRIBUTES, 
    ...
    def append_residual(self, image, gt_image, gaussians, current_batch):
        with torch.no_grad():
            deltaY, loss_mse = mse_residual(gt_image, image, gaussians.cgState.lambda_dssim)
            self.residual_state.mse_residuals[current_batch] = torch.permute(deltaY, (1, 2, 0)).flatten()

            loss_ssim = 0
            ssim_residuals = 0
            if gaussians.cgState.loss_fn == "mse_approx_ssim":
                ssim_residuals, delSSIM_delPixel, loss_ssim = self.ssim_derivative.get_ssim_residuals(image, 
                                                    gt_image, gaussians.cgState.lambda_dssim)
                
                self.residual_state.ssim_residuals[current_batch] = ssim_residuals.flatten()
    ...
    def sched_update(self, key, new_value):
        ...
        if key == "batchsize_sched" and self.batch_size != new_value:
            ...
            deltaY_batch = torch.zeros((self.batch_size, self.W*self.H*self.CHANNELS), device="cuda")
            ssim_residuals = torch.zeros((self.batch_size, self.W*self.H*self.CHANNELS), device="cuda")
            delSSIM_delPixel = torch.zeros((self.batch_size, self.W*self.H*self.CHANNELS), device="cuda")
            self.residual_state = ResidualState(deltaY_batch, ssim_residuals, delSSIM_delPixel)
            self.losses = torch.zeros((self.batch_size))
    ...
    def step(self, gaussians, line_search_lr=None):
        with torch.no_grad():
            ...
            param_list = [gaussians._opacity, gaussians._features_dc, gaussians._xyz, gaussians._scaling, gaussians._rotation]
          
            if line_search_lr != None:
                lr = line_search_lr
            else:
                lr = self.fixed_lr
                max_lr = torch.tensor(0.05,  device="cuda") if self.optim_iter < 10 else self.max_lr
                if self.auto_lr:
            ...
            if too_big:
                print(f"Iteration {self.optim_iter}: Scale is Getting to Infinity, Exiting...")
                exit(1)
            self.lrs.append(lr)
    ...
    def plot_lr(self):
        lr = [item.item() if isinstance(item, torch.Tensor) else item for item in self.lrs]
        # Create figure and axis objects using object-oriented style
        fig, ax = plt.subplots()

        # Plot the data
        ax.plot(lr, marker='.', markersize=4, linestyle='-', color='b', label='Learning Rate')
