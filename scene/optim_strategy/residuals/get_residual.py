...
def mse_residual(gt_image, image, lambda_ssim=0, return_only_loss=False):
    ...
    residual = (
        gt_image - image
    )  # torch.permute((gt_image - image), (1, 2, 0)).flatten()
...
class SimpleSSIMDerivative:
    ...
    def ssim_simple_backward(self, img1, img2, backward_dict):
        ...
        delLuminance_delPixel = (
            2 * self.GPeak * ((mu2 - mu1 * luminance) / (mu1_sq + mu2_sq + C1))
        )
        delContrastStructure_delPixel = (img2 - mu2) - contrast_structure * (img1 - mu1)
        delContrastStructure_delPixel = (
            2 * self.GPeak * delContrastStructure_delPixel
        ) / (sigma1_sq + sigma2_sq + C2)

        delSSIM_delPixel = (
            delLuminance_delPixel * contrast_structure
            + luminance * delContrastStructure_delPixel
        )
        return delSSIM_delPixel

    def get_ssim_residuals(self, rendered, gt, lambda_ssim, return_only_loss=False):
        backward_dict = self.ssim_forward(
            rendered, gt, self.window, self.window_size, self.channel
        )
