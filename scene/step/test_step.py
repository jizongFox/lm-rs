...
def get_jacobian(image, x):
    ...
    for i in range(ones.shape[0]):
        ...
        jacobian_row = torch.cat(
            tuple(map(lambda t: t.view(t.shape[0], -1), gradient_tuple)), dim=1
        ).flatten()
    ...
    with torch.no_grad():
        grad_vec = torch.cat(
            (
                ...
                gaussians._features_dc.grad.squeeze(1),
                gaussians._xyz.grad,
...
def save_jacob(
    ...
    for current_batch in range(batch_size):
        ...
        with torch.no_grad():
            J = J.cpu().numpy()
            np.savez_compressed(
                os.path.join(path, f"Jacobian_batch{current_batch}.npz"), J
            )
            residual = (torch.permute(gt_image, (1, 2, 0)) - image).flatten()
            np.savez_compressed(
                os.path.join(path, f"residual_batch{current_batch}.npz"),
                residual.cpu().numpy(),
            )
