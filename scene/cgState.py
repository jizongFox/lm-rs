...
class CGSolverState:
    ...
    def init(self, shp, dtype=torch.float32):
        return torch.zeros(
            shp, dtype=dtype, layout=torch.strided, device=torch.device("cuda")
        )
    ...
    def set_scene_size(self, scene):
        ...
        self.width_blocks = (
            self.width + (self.tile_block_dim[0] - 1)
        ) // self.tile_block_dim[0]
        self.height_blocks = (
            self.height + (self.tile_block_dim[1] - 1)
        ) // self.tile_block_dim[1]

        self.total_blocks_sampled = self.width_blocks * self.height_blocks
        self.state["sampled_pixels"] = self.init(
            (self.batch_size, self.total_blocks_sampled, self.sample_per_block),
            torch.int32,
        )
        self.state["likelihoods"] = self.init(
            (self.batch_size, self.height_blocks * self.width_blocks, 16 * 16)
        )
        self.state["n_of_gaussians_per_pixel"] = self.init(
            (self.batch_size, self.height_blocks * self.width_blocks, 16 * 16),
            torch.int32,
        )
    ...
    def update(self, key: str, value):
        if key == "cgiter_sched" and self.cg_iter != value:
            ...
            self.cg_iter = value
            self.state["d_sumResidual"] = self.init(
                (self.cg_iter,)
            )  # Used to store d_r vs iter
