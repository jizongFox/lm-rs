...
class LossSampler(BaseSampler):
    ...
    def sample(self, current_batch, cgState, **kwargs):
        ...
        if new_h != h or new_w != w:
            loss_map = torch.nn.functional.pad(
                loss_map, (0, new_w - w, 0, new_h - h), mode="constant", value=0
            )
        ...
        loss_map = (
            loss_map.view(cgState.height_blocks, BLOCK_Y, cgState.width_blocks, BLOCK_X)
            .permute(0, 2, 1, 3)
            .reshape(-1, TILE_SIZE)
        )
            ...
            probabilities_viz = (
                (
                    probabilities.reshape(
                        cgState.height_blocks, cgState.width_blocks, BLOCK_Y, BLOCK_X
                    )
                    .permute(0, 2, 1, 3)
                    .reshape(
                        cgState.height_blocks * BLOCK_Y, cgState.width_blocks * BLOCK_X
                    )
                )
                .cpu()
                .numpy()
    ...
    def visualize_loss_map(self, loss_map):
        ...
        plt.savefig(
            os.path.join(self.save_dir, f"img_{self.get_iteration()}.png"), dpi=300
        )
        plt.close(fig)
