...
for scene in "${db_scenes[@]}"; do
    SOURCE_PATH="datasets/db/${scene}"
    MODEL_PATH="output/${scene}"
    mkdir -p $MODEL_PATH
    python train.py \
            --auto_lr --max_lr 0.2 \
            --regularizer=1.0 --source_path $SOURCE_PATH --model_path $MODEL_PATH \
            --eval --iterations=400 --sh_degree=0 --log_freq=10 --log_image_freq=100 \
