...
for scene in "${mipnerf360_indoor_scenes[@]}"; do
    SOURCE_PATH="datasets/mip-nerf/${scene}"
    MODEL_PATH="output/${scene}"
    mkdir -p $MODEL_PATH
    python train.py --images="images_2" \
            --auto_lr --max_lr 0.2 \
            --regularizer=0.1 --source_path $SOURCE_PATH --model_path $MODEL_PATH \
            --eval --iterations=300 --sh_degree=0 --log_freq=10 --log_image_freq=100 \
